...
def create_config(env_name, ctrl_type, ctrl_args, overrides, logdir, env=None):
    cfg = DotMap()
    type_map = DotMap(
        exp_cfg=DotMap(
            sim_cfg=DotMap(
    ...
    loader = importlib.machinery.SourceFileLoader("finrl", os.path.join(dir_path, "finrl.py"))
    spec = importlib.util.spec_from_loader(loader.name, loader)
    cfg_source = importlib.util.module_from_spec(spec)
    loader.exec_module(cfg_source)
    cfg_module = cfg_source.CONFIG_MODULE(env)
