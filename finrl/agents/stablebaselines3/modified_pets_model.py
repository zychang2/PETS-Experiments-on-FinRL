...
class ModifiedPETSAgent:   
    ...
    def plan_action(state, models, horizon, action_dim, num_samples=100, num_elites=10, num_iterations=5):
        ...
        for _ in range(num_iterations):
            ...
            elite_rewards = rewards.sort(descending=True)[0][:num_elites]
            elite_actions = sampled_actions[elite_indices]

            action_mean = elite_actions.mean(dim=0)
            action_std = elite_actions.std(dim=0)

            if elite_rewards.mean() > best_reward:
                best_reward = elite_rewards.mean()
                best_action = action_mean
    ...
    def get_validation_sharpe(iteration): # This function is only needed for ensembling, ignore during model development portion
        df_total_value = pd.read_csv(
            f"results/account_value_validation_modified_PETS_{iteration}.csv"
        )
    ...
    def DRL_prediction(
        ...
        df_last_state.to_csv(f"results/last_state_modified_PETS_{i}.csv", index=False)
        return last_state
    ...
    def _train_window(
        ...
        print(f"======Modified PETS Training (Modified PETS)========")
        ...
        print(
            f"======Modified PETS Validation from: ",
            validation_start_date,
            "to ",
            validation_end_date,
        )
        ...
        print(f"Modified PETS Sharpe Ratio: ", sharpe)
        sharpe_list.append(sharpe)

        return models, sharpe_list, sharpe


    def run_modified_PETS_strategy(
        self,
        timesteps,
    ):
        sharpe_list = []
        sharpe = -1
        """Strategy for Modified PETS"""
        print("============Start Modified PETS Strategy============")
