...
class MBRLAgent:   
    ...
    @staticmethod
    def train_dynamics_model(data, model, optimizer, criterion, iter_num, batch_size=32):
    ...
    @staticmethod
    def plan_action(state, model, horizon, num_samples=100):
    ...
    def DRL_validation(self, model, test_data, test_env, test_obs):
        for _ in range(len(test_data.index.unique())):
            action = plan_action(test_obs, dynamics_model, self.rollout_horizon)
    ...
    def DRL_prediction(
        ...
        for i in range(len(trade_data.index.unique())): # Loops through each day in timeframe
            action = plan_action(trade_obs, dynamics_model)
            test_obs, rewards, dones, info = test_env.step(action)
